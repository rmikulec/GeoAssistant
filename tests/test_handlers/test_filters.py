...
def test_to_cql_mixed_ops():
    ...
    gf1 = GeoFilter(field="x", value=5, op="lessThan")
    assert gf1._to_cql() == quote("x < 5", safe="")

    gf2 = GeoFilter(field="y", value=10, op="greaterThanOrEqual")
...
def test_to_sql_string_escaping():
    ...
    gf = GeoFilter(field="col", value="a'b", op="equal")
    assert gf._to_sql() == "\"col\" = 'a''b'"


def test_invalid_op_raises():
    with pytest.raises(ValidationError):
        GeoFilter(field="f", value=1, op="invalidOp")
